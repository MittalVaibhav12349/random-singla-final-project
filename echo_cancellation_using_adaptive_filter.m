...
[filtered_signal , error_signal ] = lm_s(X,Xe,1024);
...
[filtered_signal , error_signal ] = nlm_s(X,Xe,1024);
...
[filtered_signal , error_signal ] = rl_s(X,Xe);
...
%% Adaptive filter with subband algorithm
[filtered_signal, error_signal] = subband_af(X, Xe, 1024, 4, 0.1, 0.1);

figure % Visualization
subplot(2,1,1)
plot(filtered_signal);
title('Subband AF output and its spectrogram');
xlabel('Sample');
ylabel('Amplitude');
subplot(2,1,2)
spectrogram(filtered_signal);
...
filename = 'subbandoutput_code.wav'; % file name
audiowrite(filename,filtered_signal,Fs); % Save the audio in local file
